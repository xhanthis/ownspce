...
import type { KanbanPage as KanbanPageType, KanbanColumn, KanbanCard, PageIndexRow } from '@/lib/types'

function uid() { return Math.random().toString(36).slice(2, 10) }

interface Props { page: PageIndexRow }

export default function KanbanPage({ page }: Props) {
  ...
  useEffect(() => {
    const stored = loadPage(page.id) as KanbanPageType | null
    if (stored?.type === 'kanban') {
      setTitle(stored.title)
      setColumns(stored.columns)
    }
  }, [page.id])

  const persist = useCallback((cols: KanbanColumn[], t: string) => {
    setSaveStatus('saving')
    const data: KanbanPageType = {
      id: page.id, type: 'kanban', title: t,
      updatedAt: new Date().toISOString(), columns: cols,
    }
  ...
  function update(cols: KanbanColumn[], t = title) {
    setColumns(cols)
    debouncedPersist(cols, t)
  }

  function addColumn() {
    update([...columns, { id: uid(), title: 'New Column', cards: [] }])
  }

  function renameColumn(colId: string, name: string) {
    update(columns.map(c => c.id === colId ? { ...c, title: name } : c))
  }

  function deleteColumn(colId: string) {
    update(columns.filter(c => c.id !== colId))
  }

  function addCard(colId: string, cardTitle: string) {
    if (!cardTitle.trim()) return
    update(columns.map(c => c.id === colId
      ? { ...c, cards: [...c.cards, { id: uid(), title: cardTitle.trim() }] }
      : c
    ))
  }
  ...
  function findCardColumn(cardId: string): string | null {
    return columns.find(c => c.cards.find(k => k.id === cardId))?.id ?? null
  }
  ...
  function handleDragOver(e: DragOverEvent) {
    ...
    const toColId = findCardColumn(over.id as string) ?? (columns.find(c => c.id === over.id)?.id ?? null)
    ...
    const newCols = columns.map(c => {
      if (c.id === fromColId) return { ...c, cards: c.cards.filter(k => k.id !== active.id) }
      if (c.id === toColId) {
        const cards = [...c.cards]
  ...
  function handleDragEnd(e: DragEndEvent) {
    ...
    if (fromColId === toColId) {
      ...
      update(columns.map(c => c.id === fromColId ? { ...c, cards } : c))
    } else {
      debouncedPersist(columns, title)
    }
  }
  ...
  return (
    <div className="p-8 fade-in">
      <div className="flex items-center justify-between mb-6">
        <input
          value={title}
          onChange={e => { setTitle(e.target.value); debouncedPersist(columns, e.target.value) }}
...
function KanbanColumnComponent({ column, onRename, onDelete, onAddCard, onDeleteCard }: {
  column: KanbanColumn
    ...
    <div className="shrink-0 w-64 flex flex-col gap-3 rounded-card bg-[#F2F2F2] dark:bg-zinc-900 p-3 border border-[#E0E0E0] dark:border-zinc-800">
      ...
      <div className="flex items-center gap-2">
        {editing ? (
          ...
          <button onClick={() => setEditing(true)} className="flex-1 text-sm font-semibold text-left hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
            {column.title}
          </button>
        )}
        <span className="text-xs text-zinc-400">{column.cards.length}</span>
        <button onClick={() => onDelete(column.id)} className="text-zinc-300 hover:text-red-400 text-sm transition-colors">×</button>
      </div>

      {/* Cards */}
      <SortableContext items={column.cards.map(k => k.id)} strategy={verticalListSortingStrategy}>
        <div className="flex flex-col gap-2 min-h-8">
          {column.cards.map(card => (
            <KanbanCardComponent key={card.id} card={card} onDelete={() => onDeleteCard(column.id, card.id)} />
          ))}
        </div>
      </SortableContext>
      ...
      {addingCard ? (
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-1.5">
            <button onClick={submitCard} className="px-2 py-1 text-xs font-medium bg-[#0A0A0A] dark:bg-white text-white dark:text-[#0A0A0A] rounded-input hover:opacity-80">Add</button>
            <button onClick={() => setAddingCard(false)} className="px-2 py-1 text-xs text-zinc-400 hover:text-zinc-600">Cancel</button>
          </div>
        </div>
      ) : (
        <button onClick={() => setAddingCard(true)} className="text-xs text-zinc-400 hover:text-[#0A0A0A] dark:hover:text-white text-left transition-colors py-1">
          + Add card
        </button>
      )}
    </div>
...
function KanbanCardComponent({ card, onDelete }: { card: KanbanCard; onDelete: () => void }) {
  ...
  return (
    ...
    >
      ...
      {card.notes && <p className="text-xs text-zinc-400 mt-0.5">{card.notes}</p>}
      <button
