...
export default function Sidebar() {
  ...
  const grouped = PAGE_TYPES.map(pt => ({
    ...pt,
    pages: pages.filter(p => p.page_type === pt.type),
      ...
      <aside className="w-56 shrink-0 h-screen sticky top-0 flex flex-col border-r border-[#E0E0E0] dark:border-zinc-800 bg-[#F2F2F2] dark:bg-zinc-900">
        ...
        <div className="px-4 py-5">
          <Link href="/dashboard" className="flex items-center">
            <Image src="/logo.svg" alt="ownspce" width={120} height={30} className="dark:hidden" priority />
            <Image src="/logo-dark.svg" alt="ownspce" width={120} height={30} className="hidden dark:block" priority />
          </Link>
        </div>

        {/* New Page */}
        <div className="px-3 mb-3">
          <Button variant="primary" size="sm" className="w-full" onClick={() => setNewPageOpen(true)}>
            + New Page
          </Button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 pb-4 flex flex-col gap-4">
          {grouped.map(group => (
            <div key={group.type}>
              <p className="px-2 mb-1 text-xs font-medium text-zinc-400 uppercase tracking-wider">{TYPE_LABELS[group.type]}</p>
              {group.pages.map(page => (
                <Link
                  key={page.id}
                  href={`/pages/${page.id}`}
                  className={`flex items-center px-2 py-1.5 rounded-input text-sm truncate transition-colors ${pathname === `/pages/${page.id}` ? 'bg-white dark:bg-zinc-800 font-medium' : 'hover:bg-white dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400'}`}
                >
                  {page.title}
                </Link>
              ))}
        ...
        <div className="border-t border-[#E0E0E0] dark:border-zinc-800 px-3 py-3">
          <Link href="/profile" className="block px-2 py-1.5 text-sm text-zinc-500 dark:text-zinc-400 hover:text-[#0A0A0A] dark:hover:text-white rounded-input hover:bg-white dark:hover:bg-zinc-800 transition-colors">
            Profile
          </Link>
        </div>
      </aside>
